 ...
 */
export function bd09togcj02(bd_lng: number, bd_lat: number): Coordinate {
  const lng = +bd_lng;
  const lat = +bd_lat;
  const x = lng - 0.0065;
  const y = lat - 0.006;
...
export function gcj02tobd09(lng: number, lat: number): Coordinate {
  const latNum = +lat;
  const lngNum = +lng;
  const z = Math.sqrt(lngNum * lngNum + latNum * latNum) + 0.00002 * Math.sin(latNum * x_PI);
  const theta = Math.atan2(latNum, lngNum) + 0.000003 * Math.cos(lngNum * x_PI);
...
export function wgs84togcj02(lng: number, lat: number): Coordinate {
  ...
  if (out_of_china(lngNum, latNum)) {
    ...
    let dlat = transformlat(lngNum - 105.0, latNum - 35.0);
    let dlng = transformlng(lngNum - 105.0, latNum - 35.0);
    const radlat = latNum / 180.0 * PI;
    let magic = Math.sin(radlat);
    magic = 1 - ee * magic * magic;
    const sqrtmagic = Math.sqrt(magic);
    dlat = (dlat * 180.0) / ((a * (1 - ee)) / (magic * sqrtmagic) * PI);
    dlng = (dlng * 180.0) / (a / sqrtmagic * Math.cos(radlat) * PI);
    const mglat = latNum + dlat;
    const mglng = lngNum + dlng;
    return [mglng, mglat];
  }
}
...
function transformlat(lng: number, lat: number): number {
  const latNum = +lat;
  const lngNum = +lng;
  let ret = -100.0 + 2.0 * lngNum + 3.0 * latNum + 0.2 * latNum * latNum + 0.1 * lngNum * latNum + 0.2 * Math.sqrt(Math.abs(lngNum));
...
function transformlng(lng: number, lat: number): number {
  const latNum = +lat;
  const lngNum = +lng;
  let ret = 300.0 + lngNum + 2.0 * latNum + 0.1 * lngNum * lngNum + 0.1 * lngNum * latNum + 0.1 * Math.sqrt(Math.abs(lngNum));
  ret += (20.0 * Math.sin(6.0 * lngNum * PI) + 20.0 * Math.sin(2.0 * lngNum * PI)) * 2.0 / 3.0;
...
// UMD global export for browser compatibility
